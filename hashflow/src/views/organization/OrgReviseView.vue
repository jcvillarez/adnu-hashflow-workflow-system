...
.indv-popup-password-frame > .table-frame > table > tbody > tr > td > textarea {
  @apply border-[1px] border-[#aaa] h-[275px] min-h-[70px] max-h-[275px] p-[10px];
}
        ...
        >
          <router-link
            :to="{
              ...
              query: {
                orgApprovalId: this.$route.query.orgApprovalId,
                verdict: decision,
...
export default {
  ...
  methods: {
    ...
    reviseDocument() {
      ...
      try {
        if (currentUser != null) {
          const ReviseDocument = Parse.Object.extend("Org_Revise_Document");
